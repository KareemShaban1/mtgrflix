  ...
  function setupStarRating(container, ratingTextElement) {
    ...
    stars.forEach((star, index) => {
      ...
      star.addEventListener("mouseover", function () {
        ...
        stars.forEach(s => s.style.color = s.getAttribute("data-value") <= value ? "gold" : "gray");
      });
  ...
  function initializeProductModal(modal, submitButtonSelector) {
    ...
    if (submitBtn) {
      submitBtn.addEventListener('click', function (event) {
        ...
        const rating = activeStars.length > 0 ? activeStars[activeStars.length - 1].getAttribute('data-value') : null;
          ...
          .then(data => {
            if (data.success) {
              ...
              if (currentIndex < products.length - 1) {
                ...
                const storeModalElement = document.querySelector('[id^="modal-rate-store"]');
                const storeModal = bootstrap.Modal.getOrCreateInstance(storeModalElement);
                storeModal.show();
        ...
        fetch(`/api/submit-testimonial`, {
          ...
          .then(data => {
            if (data.success) {
              ...
              const thankYouModal = new bootstrap.Modal(document.querySelector('[id^="modal-thank-you"]'));
              
              // Update modal content with coupon if available
              if (data.coupon) {
                const couponElement = document.getElementById('coupon-display');
                if (couponElement) {
                  couponElement.textContent = data.coupon; // or format as needed
                  couponElement.style.display = 'block'; // Make it visible
                  ...
                  if (discountElement && data.coupon.discount) {
                    discountElement.textContent = `(${data.coupon.discount}% OFF)`;
                  }
                }
              }
